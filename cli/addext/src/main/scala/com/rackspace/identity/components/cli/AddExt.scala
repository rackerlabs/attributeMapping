...
object AddExt {
  ...
  def parseArgs(args: Array[String], // scalastyle:ignore
                ...
                in: InputStream,
                out: PrintStream,
                err: PrintStream): Option[(Source, Source, Destination, Boolean, String, Boolean)] = {

    val parser = new ArgotParser("addext", preUsage=Some(s"$title v$version"))

    val res = parser.parameter[String]("authResult",
                                        "A keystone auth \"access\" result.",
                                        false)
    ...
    def authSource : Source = new StreamSource(URLResolver.toAbsoluteSystemId(res.value.get, base))
    ...
    try {
      ...
      if (printVersion.value.getOrElse(false)) {
        ...
        Some((authSource, samlSource, destination, !dontValidate.value.getOrElse(false),
